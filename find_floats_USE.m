function [PlotMapErr,PlotCoverage,float_locations,Irange,Jrange]=find_floats_USE(num_iter)
...
imax=1128;
igap=6;
% make sure that the range is within the indices that we have decorrelation
% lengths for
...
%Depth(Depth<2000)=nan;
Depthss = Depth(Irange,Jrange);
Depthssv = Depthss(:);
Depthsssv = Depthssv(IsOcean);
...
for iter=1:num_iter
    ...
    for test_ind=1:NM
        ...
        if gain > best_gain && Depthsssv(test_ind)>=2000
