function [PlotMapErr,PlotCoverage,float_locations,Irange,Jrange]=cov_float(num_iter)
...
imin=1;
imax=1128; %range of i points
igap=24;
jmin=1;
jmax=336; %range of j points
jgap=24;
...
%make sure that DCL is good and prepped, no nans, complex or zeros
% semimajor lengths
major=real(DCL(:,:,1));
            ...
            for I=imin:igap:imax
                for J=jmin:jgap:jmax
                    ...
                    %if dx <= 1500 && dy <= 350 && (dx.^2 + dy.^2) < 1000000
                        ...
                        CC(I,J)=covmatrix(2,1)*sqrt(exp(-((dx.*cos(dctheta)+dy.*sin(dctheta))^2/dca.^2+(dx.*sin(dctheta)+dy.*cos(dctheta))^2/dcb.^2)));
                    %else
                        %CC(I,J)=0;
                    %end
                end
...
%     hold off
    if max_ij(1)==0
        ...
        for num = 1:ND
            ...
            for nx=1:NX
                for ny=1:NY
                    i=round((nx-1)*igap+imin); % CHECK MATH
                    j=round((ny-1)*jgap+jmin); % CHECK MATH
                    a = (xk(I) - xk(i));
                    dx = a*cosd((y(J) + y(j))*.5);% scaled distance in X
                    dy = yk(J) - yk(j);
                    ...
                    DIST(nx,ny,num)=exp(-((dx.*cos(dctheta)+dy.*sin(dctheta))^2/dca.^2+(dx.*sin(dctheta)+dy.*cos(dctheta))^2/dcb.^2)); %gaussian distances
                end
            end
        end
